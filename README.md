...
Ferox is a Rust-native offensive security framework for authorized operators who need uncompromising speed, safety, and observability.
...
| Evasion & Post | EDR bypass research, session hijacking, credential workflows |
| Memory Forensics | Dump parsing, process scoring, malware heuristics, MITRE correlation |
| Governance | Authorization, safe mode, tamper-proof audit logging |
...
# Recommended: build with memory forensics support
cargo build --release --features memory-forensics

# Optional: initialize a local workspace and default policy
...
Run sensitive modules with full auditing without touching live infrastructure.
```bash
...
./target/release/ferox memory analyze dumps/workstation.dmp --database analysis.db --output reports/workstation.json
./target/release/ferox memory malfind dumps/workstation.dmp --min-score 0.6 --mitre --format table
./target/release/ferox memory mitre dumps/workstation.dmp --database analysis.db --format markdown
```
...
Tip: Add `SAFE_MODE=1` for smoke validation in CI environments.
...
- [docs/changelog.md](docs/changelog.md) — release evolution
...
Version 2.0.0 · Updated 2025-11-12 · Ferox — Fast. Fierce. Fearless. 🦊
# 🦊 Ferox 2.0.0 — Fast, Fierce, Fearless
