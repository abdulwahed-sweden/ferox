...
export const moduleToasts = {
  ...
  privescComplete: (toastId: string, vectorsFound: number, escalated: boolean) => {
    toast.dismiss(toastId);
    if (escalated) {
      ...
      toast.success(`Found ${vectorsFound} potential vector${vectorsFound > 1 ? 's' : ''}`, {
        duration: 4000,
        icon: '🔍',
      });
    } else {
  ...
  discoveryComplete: (hostsFound: number) => {
    toast.success(`Discovery complete: ${hostsFound} host${hostsFound !== 1 ? 's' : ''} found`, {
      duration: 4000,
      icon: '🌐',
    });
  },
