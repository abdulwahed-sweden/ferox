...
export function PayloadBuilder() {
  ...
  const filteredFormats = formats.filter(
    (f) => f.os.includes(config.target_os) || f.os.includes("universal"),
  );
  ...
  const getLogIcon = (level: string) => {
    switch (level) {
      ...
      case "warn":
        return <AlertTriangle size={14} className="text-warning-text" />;
      default:
        return <Info size={14} className="text-info-text" />;
    }
  };
    ...
    <div className="h-full flex flex-col bg-dark-900">
      ...
      <div className="p-4 border-b border-dark-600 bg-dark-800">
        ...
        <p className="text-xs text-text-muted mt-1">
          Generate simulated payload metadata for demo/training. No actual
        ...
        <div className="w-80 border-r border-dark-600 overflow-y-auto p-4 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-2">
            ...
            <div>
              ...
              <input
                ...
                value={config.lport}
                onChange={(e) =>
                  handleChange("lport", parseInt(e.target.value) || 4444)
                }
                placeholder="4444"
                className="w-full px-3 py-2 bg-dark-700 border border-dark-600 rounded text-sm text-text-primary placeholder:text-text-muted focus:border-ferox-green/50 focus:outline-none"
              />
          ...
          <div>
            ...
            <input
              ...
              value={config.name || ""}
              onChange={(e) =>
                handleChange("name", e.target.value || undefined)
              }
              ...
              <CollapsibleSection
                ...
                badge={
                  <span className="text-xs text-warning-text">
                    Est. Detection:{" "}
                    {(
                      result.detection_analysis.estimated_detection_rate * 100
                    ).toFixed(0)}
                    %
                  </span>
                }
                ...
                <div className="space-y-3">
                  <div>
                    ...
                    <div className="flex flex-wrap gap-2">
                      {result.detection_analysis.likely_detectors.map(
                        (d, i) => (
                          <span
                            key={i}
                            className="px-2 py-1 bg-danger-soft text-danger-text rounded text-xs"
                          >
                            {d}
                          </span>
                        ),
                  ...
                  <div>
                    ...
                    <ul className="space-y-1">
                      {result.detection_analysis.behavioral_indicators.map(
                        (ind, i) => (
                  ...
                  <div>
                    ...
                    <ul className="space-y-1">
                      {result.detection_analysis.evasion_notes.map(
                        (note, i) => (
                          <li key={i} className="text-xs text-text-muted">
                            • {note}
                          </li>
                  ...
                  {result.mitre_mapping.map((mapping, i) => (
                    <div key={i} className="bg-dark-900 rounded p-3">
                      ...
                      <div className="flex items-center gap-2 mt-1">
                        ...
                        <span className="text-xs text-ferox-green">
                          {mapping.tactic}
                        </span>
                      </div>
                      <p className="text-xs text-text-muted mt-1">
                  ...
                  {result.execution_hints.map((hint, i) => (
                    <div key={i} className="bg-dark-900 rounded p-3">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <button
                          onClick={() =>
                            copyToClipboard(hint.command, hint.name)
                          }
